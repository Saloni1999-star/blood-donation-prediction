...
4.  **Automated Machine Learning (AutoML) - TPOT:**
...
6.  **Model Interpretation:**
    * The coefficients of the Logistic Regression model were analyzed to understand the impact of each feature on the prediction of blood donation. A bar plot of coefficients was generated for visual interpretation.
...
* **TPOT Best Model Performance:**
    ...
    * **Best Pipeline Found by TPOT:**
        `Pipeline(steps=[('RobustScaler', RobustScaler()), ('MLPClassifier', MLPClassifier(alpha=0.0001, learning_rate_init=0.01))])`
    * **Interpretation:**
        The TPOT classifier achieved an overall accuracy of 70%, which is lower than the Logistic Regression model's 66% accuracy. Looking at the metrics for class 1 (donors), TPOT's performance, while not completely zero like before, still heavily favors the majority class:
...
1.  **Open in Codespaces:** Navigate to this repository on GitHub and click the green "Code" button, then select "Open with Codespaces". A cloud-based development environment will launch.
2.  **Open `main.ipynb`:** Once the Codespace is ready, open the `main.ipynb` file in the file explorer.
3.  **Install Dependencies:** Open the terminal (`Terminal > New Terminal`) and install any missing libraries (e.g., `pip install tpot imbalanced-learn matplotlib seaborn`).
4.  **Run Cells:** Execute each cell in the `main.ipynb` notebook sequentially.
...
5.  **Run Cells:** Execute each cell in the notebook sequentially.